...
public class GetTripsNumWithExactStopsProcessor extends Processor {
	public final static String KEY = "4";
	private final static String EXACT_STOP_INSTRUCTION = 
		" *Please input command " + KEY + " to query the number of trips from one town to another by exact number of stops\n";

	public GetTripsNumWithExactStopsProcessor(UIControlCenter uiCenter){
		super(uiCenter);
	}
	...
	public boolean process(String cmd) {
		String errMsg = "logical error occurs in GetTripsNum with exact stops processor\n";
		String instruction = "\nInput two towns' names and the exact stops between them separated by \'" 
			+ Processor.SEP 
			+ "\', for example A-B-5.  "
	...
	protected void callDataService(String cmd){
		...
		if(values != null){
			...
			int result = dataService.getTripsNumWithExactStops(values[0], values[1], Integer.parseInt(values[2]));
